  ...
  describe '#create' do
    context 'when the attributes are correct' do
      it 'renders status 200' do
        # allow(SecureRandom).to receive(:uuid).and_return('2d931510-d99f-494a-8c67-87feb05e1594')
        # user = create(:user)
        # playlist = Playlist.create(title: 'Title', user_id: user.id, uuid: SecureRandom.uuid)
        # valid_attributes = { title: playlist.title, user_id: playlist.user_id, uuid: SecureRandom.uuid }
